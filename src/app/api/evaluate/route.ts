...
function matchKnowledgePoints(inputKps: string[]): string[] {
  ...
  for (const inputKp of inputKps) {
    ...
    for (let i = 0; i < KNOWLEDGE_POINTS.length; i++) {
      ...
      const matchedWords = kpWords.filter(word => inputLower.includes(word));
      const score = matchedWords.length / kpWords.length;
    ...
    if (bestMatchIdx >= 0 && bestMatchScore >= 0.5) {
      matched.push(KNOWLEDGE_POINTS[bestMatchIdx]);
      usedIndices.add(bestMatchIdx);
    } else {
      matched.push(inputKp);
    }
  }
...
【易错知识点参考库】（请严格从此列表中选择与代码实际错误相符的项，不要随意添加不在列表中的内容；如果代码无相关易错点，返回空数组）
${KNOWLEDGE_POINTS.map((kp, i) => `${i + 1}. ${kp}`).join('\n')}

【输出要求】
- 必须返回**纯 JSON**，不要包含任何 markdown 代码块标记或额外文字
- knowledgePoints（易错知识点）必须严格从【易错知识点参考库】中选择与代码实际表现相符的项，只选真正与代码问题相关的，不要为了凑数硬塞
- 特别提醒：如果代码里没有 for/while/if/def 等结构，绝对不要选相关语句的知识点；如果代码里没有字典，不要选字典相关项；依此类推
...
- knowledgePoints: ["本次实际体现的易错知识点 1", "本次实际体现的易错知识点 2", ...]，必须严格选自【易错知识点参考库】；无则返回空数组；最多返回 5 个
